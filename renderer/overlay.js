...
function resize() {
  ...
  const dpr = Math.max(Math.min(1, nativeDpr), nativeDpr * renderBudget.scale);
  W = innerWidth;
  H = innerHeight;
...
const { COL, applyBody, drawDorsal, drawSide, morphOf } = createFlyArt(ctx);
...
function drawFly(fly, now) {
  ...
  const morph = morphOf(fly.geneD, fly.geneP, fly.geneG, fly.geneX, fly.geneY);
  const pose = flying ? 'fly' : grooms ? 'groom' : 'rest';
  const sprite = !performanceMode || flies.length < 64 || morph === 'rainbow' ? null : sprites.get(morph, fly.sex, pose);
  if (sprite) {
    const phase = flying ? now * 0.55 + fly.seed : grooms ? now * 0.028 : 0;
    const frame = ((Math.floor(phase / (Math.PI * 2) * SPRITE_FRAMES) % SPRITE_FRAMES) + SPRITE_FRAMES) % SPRITE_FRAMES;
    ctx.drawImage(sprite, frame * SPRITE_SIZE, 0, SPRITE_SIZE, SPRITE_SIZE, -8, -8, 16, 16);
  } else {
...
function drawRag() {
  ...
  drawTexturedRag(ctx, mouse.x, mouse.y, { wiping: ragWipe });
}
...
function frame(now) {
  ...
  try {
    ...
    tickFanfare(now);
    draw(now);
    if (renderBudget.sample({ interval, work: performance.now() - started,
      population: flies.length, dpr: Math.min(devicePixelRatio || 1, 2), now,
      enabled: performanceMode && !swatterOn && !ragOn && !fanfareUntil })) resize();
    lifeAcc += dt;
    if (lifeAcc > 0.4) {
      lifeAcc = 0;
      publishLife();
...
if (api) {
  ...
  api.onCmd((d) => {
    if (d.name === 'performance') performanceMode = !!d.value;
